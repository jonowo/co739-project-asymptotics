...
theorem IsChain.le_or_lt_of_mem_of_mem (hC : IsChain C) (hxC : x ∈ C) (hyC : y ∈ C) :
    x ≤ y ∨ y < x := by
  rcases le_or_le_of_mem_of_mem hC hxC hyC with h | h
  · left
    exact h
...
theorem IsChain.insert_UB (hC : IsChain C) (hb : IsUB C b) : IsChain (insert b C) := by
  ...
  · rcases h' with rfl | h'
    · left
      exact hb x' h
    · exact le_or_le_of_mem_of_mem hC h h'
...
theorem IsChain.subset_isChain {C S : Set α} (hC : IsChain C) (hSC : S ⊆ C) : IsChain S := by
  ...
  exact fun x y hx hy ↦ hC x y (mem_of_mem_of_subset hx hSC) (mem_of_mem_of_subset hy hSC)
...
-- Can you prove this in one line using a term?
  ⟨hSD, hD, fun x y hxD hyS hxy ↦ h.mem_of_mem_of_le (mem_of_mem_of_subset hxD hDC) hyS hxy⟩

-- even this can be a one-liner.
theorem IsSegmentOf.trans (h : IsSegmentOf S C) (h' : IsSegmentOf C D) : IsSegmentOf S D := by
  refine ⟨?_, ?_, ?_⟩
  · exact subset_trans h.subset h'.subset
  · exact h'.chain_right
  intro x y hxD hyS hxy
  refine h.mem_of_mem_of_le ?_ hyS hxy
  refine h'.mem_of_mem_of_le hxD ?_ hxy
  exact mem_of_mem_of_subset hyS h.subset
...
theorem IsSegmentOf.exists_strictUB_mem_of_ne (h : IsSegmentOf S U) (hne : S ≠ U) :
  ...
  have h_ssubset : S ⊂ U := by
    exact ssubset_iff_subset_ne.mpr ⟨h.subset, hne⟩
  ...
  have le_or_lt : d ≤ c ∨ c < d := by
    have hUChain : IsChain U := h.chain_right
    refine hUChain.le_or_lt_of_mem_of_mem hdU ?_
    exact mem_of_mem_of_subset hcS h.subset
...
theorem GoodWRT.insert_ub (b : Set α → α) (hb : ∀ C, IsChain C → IsStrictUB C (b C))
  ...
  have hbCS : b C ∉ S := by
    ...
    intro c h'
    refine hSeg.mem_of_mem_of_le ?_ hbCS ?_
    · right
      exact h'
    · apply le_of_lt
      exact (hb C h.1) c h'
  ...
  have hSC : IsSegmentOf S C := by
    exact hSeg.subset_right h.1 hS (subset_insert _ _)
  ...
  · -- Since `b C ≤ y ∈ S ∪ {b s} ⊆ C`, this contradicts `b C` being a strict UB for `C`.
    have h' : y ∈ C := mem_of_subset_of_mem hSeg'.subset hy
...
theorem zorn (h : ∀ C, IsChain C → ∃ (b : α), IsUB C b) : ∃ (m :α ), IsMaximal m := by
  ...
  have h_strictUB : ∀ (C : Set α), ∃ (b : α), IsChain C → IsStrictUB C b := by
    ...
    · -- use `h` and `h_con` to find a strict upper bound.
      rcases h C hC with ⟨b₀, h₀⟩
      rcases h_con b₀ with ⟨b₁, h₁⟩
      have h₂ : b₀ < b₁ := by exact lt_iff_le_and_ne.mpr h₁
      use b₁
      intro _ c' h'
      exact lt_of_le_of_lt (h₀ c' h') h₂
  ...
  have forall_good_segment : ∀ D, GoodWRT b D → IsSegmentOf D U := by
    ...
    intro x y hx hy hxy
    rcases mem_sUnion.mp hx with ⟨E, h'', h'''⟩
    dsimp at h''

    have h₀ : IsSegmentOf E D ∨ IsSegmentOf D E := by
      exact goodWRT_comparability hb_not_mem h' h''

    rcases h₀ with h₀ | h₀
    · exact h₀.subset h'''
    exact IsSegmentOf.mem_of_mem_of_le h₀ h''' hy hxy
  ...
  have hU_good : GoodWRT b U := by
    ...
    have hSD : S ⊆ D := by
      intro s h'
      refine hDU.mem_of_mem_of_le (hS.subset h') hdD ?_
      exact le_of_lt (hSd s h')

    have hSD_seg : IsSegmentOf S D := by
      exact hS.subset_right hD.1 hSD hDU.subset
  ...
  exact (insert_subset_iff.mp h_ins).left
